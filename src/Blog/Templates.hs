...
module Blog.Templates(templatesR) where
...
templatesR :: H.Rules ()
templatesR =
  H.match "templates/*.html" $ H.compile H.templateCompiler

flameR :: H.Rules ()
flameR =
  let flameTemplate :: String
      flameTemplate = renderHtml flame
  in  H.match "templates/flame.haml" $ do
      H.route $ H.gsubRoute "pages/" (const "") `H.composeRoutes` H.setExtension "html"
      H.compile $ H.makeItem (H.readTemplate flameTemplate)
